...
def main():
    # File paths
    asset_file = 'data/assets/facility_database_with_regions.csv'
    emission_factors_file = 'data/assumptions/emission_factors.csv'
    benchmarks_file = 'data/assumptions/product_benchmarks.csv'
    ...
    output_path = 'outputs/energy_emission_analysis.csv'
    ...
    def save_barplot(series, title, xlabel, ylabel, filename, color_palette='viridis'):
        ...
        sns.barplot(x=series.values, y=series.index, palette=color_palette)
        plt.title(title)
        plt.xlabel(xlabel)
        plt.ylabel(ylabel)
        plt.tight_layout()
    ...
    loc_agg.to_csv('outputs/emissions_by_location_summary.csv')
    comp_agg.to_csv('outputs/emissions_by_company_summary.csv')
