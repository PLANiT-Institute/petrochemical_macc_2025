...
def load_all_data():
    ...
    elif (OUTPUTS_DIR / 'scenario_summary_v3.csv').exists():
        data['scenarios_6'] = pd.read_csv(OUTPUTS_DIR / 'scenario_summary_v3.csv')
    else:
        ...
        if scenario_data and scenario_data.get('facilities') is not None:
            ...
            if deployment is not None:
                ...
                for year in [2025, 2030, 2035, 2040, 2045, 2050]:
                    ...
                    if len(year_data) > 0:
                        d = year_data.iloc[0]
                        # Lookup grid EF from data
                        grid_ef_val = data['grid_ef'][data['grid_ef']['year'] == year]['grid_ef_tco2_per_mwh'].values[0]
                        ws.cell(row=row, column=1, value=year)
                        ws.cell(row=row, column=2, value=f"{d['bau_mt']:.2f}")
                        ws.cell(row=row, column=3, value=f"{d['total_deployed_mt']:.2f}")
                        ws.cell(row=row, column=4, value=f"{d['actual_emissions_mt']:.2f}")
                        ws.cell(row=row, column=5, value=f"{grid_ef_val:.3f}")
                        row += 1
