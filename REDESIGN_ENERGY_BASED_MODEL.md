...
# 🚨 CRITICAL: 납사 원료비는 고정비 (Before/After 양쪽 다 동일)
# 납사 원료비는 계산에서 완전히 제외됨 (상쇄)
# ONLY 연료비만 비교!

h2_fuel_cost = h2_consumption_ton * 1000 * h2_price_per_kg  # $/ton ethylene
naphtha_fuel_cost = naphtha_combustion_gj * naphtha_price_per_gj  # $/ton ethylene (연료용만!)

fuel_diff_per_ton = h2_fuel_cost - naphtha_fuel_cost
...
**🚨 CRITICAL NOTE**:
- **납사 원료비 (feedstock)는 고정비**: Before/After 양쪽 다 동일하게 들어가므로 계산에서 완전히 제외
- **납사 연료비 (fuel)만** 수소 비용과 비교
- 원료용 납사는 에틸렌 생산에 필수이며 H2/전기 전환과 관계없이 계속 구매해야 함
...
electricity_cost = 13 * electricity_price_per_mwh  # $/ton ethylene
naphtha_fuel_cost = 105 * naphtha_price_per_gj  # $/ton ethylene (연료용만!)

fuel_diff_per_ton = electricity_cost - naphtha_fuel_cost
...
**🚨 CRITICAL NOTE**:
- **납사 원료비는 고정비**: 계속 구매하므로 계산에서 완전히 제외
- **납사 연료비만** 전기 비용과 비교
