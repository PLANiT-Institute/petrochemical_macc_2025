...
def load_data():
    ...
    regional_mac = pd.read_csv(OUTPUTS_DIR / 'regional_mac_summary.csv')
    stranded_assets = pd.read_csv(OUTPUTS_DIR / 'stranded_assets_summary.csv')
    ...
    try:
        tech_params = pd.read_csv(DATA_DIR / 'assumptions/technology_parameters.csv', on_bad_lines='skip')
    except Exception:
...
def plot_macc_curves(df, output_path):
    ...
    # CSV export: MACC curves
    macc_rows = []
    for scenario in sorted(df['scenario'].unique()):
        scenario_data = df[(df['scenario'] == scenario) & (df['year'] == 2050)]
        scenario_data = scenario_data[scenario_data['mac_usd_per_tco2'] > 0].sort_values('mac_usd_per_tco2')
        if len(scenario_data) > 0:
            cum = scenario_data['abatement_tco2'].cumsum() / 1e6
            for ca, mac in zip(cum, scenario_data['mac_usd_per_tco2']):
                macc_rows.append({'category': scenario, 'value': mac,
                                  'subcategory': 'cumulative_abatement_MtCO2',
                                  'unit': 'USD_per_tCO2'})
...
def plot_emissions_trajectory(df, output_path):
    ...
    save_figure(plt.gcf(), output_path)
    print(f"Saved: {output_path}")
    # CSV export: emissions trajectory
    traj_rows = []
    for scenario in sorted(df['scenario'].unique()):
        scenario_data = df[df['scenario'] == scenario].groupby('year')['emissions_tco2'].sum().reset_index()
        scenario_data['emissions_mt'] = scenario_data['emissions_tco2'] / 1e6
        for _, row in scenario_data.iterrows():
            traj_rows.append({'year': int(row['year']), 'category': scenario,
                              'value': row['emissions_mt'], 'unit': 'MtCO2_per_yr'})
    save_figure_data(pd.DataFrame(traj_rows), output_path, figure_type='line')
...
def plot_regional_heatmap(df, output_path):
    ...
    save_figure(plt.gcf(), output_path)
    print(f"Saved: {output_path}")
    # CSV export: regional heatmap
    pivot_data = df.groupby(['year', 'region'])['emissions_tco2'].sum().reset_index()
    pivot_data['value'] = pivot_data['emissions_tco2'] / 1e6
    tidy = pivot_data.rename(columns={'region': 'category'})[['year', 'category', 'value']]
    tidy['unit'] = 'MtCO2_per_yr'
    save_figure_data(tidy, output_path, figure_type='heatmap')
...
def plot_technology_timeline(df, output_path):
    ...
    save_figure(plt.gcf(), output_path)
    print(f"Saved: {output_path}")
    # CSV export: technology timeline
    deployed = df[df['tech_deployed'] == 1].copy()
    tech_counts = deployed.groupby(['year', 'technology']).size().reset_index(name='value')
    tidy = tech_counts.rename(columns={'technology': 'category'})
    tidy['unit'] = 'facility_count'
    save_figure_data(tidy, output_path, figure_type='stacked_bar')
...
def plot_h2_vs_elec_comparison(df, output_path):
    ...
    save_figure(plt.gcf(), output_path)
    print(f"Saved: {output_path}")
    # CSV export: H2 vs Elec comparison (2 panels)
    h2e_rows = []
    # Panel A: Cumulative CAPEX
    for scenario in sorted(df['scenario'].unique()):
        s_data = df[df['scenario'] == scenario].groupby('year')['capex_usd'].sum().cumsum() / 1e9
        for yr, val in s_data.items():
            h2e_rows.append({'year': int(yr), 'category': scenario, 'value': val,
                             'unit': 'billion_USD', 'panel': 'A'})
    # Panel B: Energy demand 2050
    final_2050 = df[df['year'] == 2050].groupby('scenario').agg(
        {'h2_demand_t': 'sum', 'elec_demand_mwh': 'sum'}).reset_index()
    for _, row in final_2050.iterrows():
        h2e_rows.append({'category': row['scenario'], 'subcategory': 'H2 Demand',
                         'value': row['h2_demand_t'] / 1e6, 'unit': 'Mt', 'panel': 'B'})
        h2e_rows.append({'category': row['scenario'], 'subcategory': 'Elec Demand',
                         'value': row['elec_demand_mwh'] / 1e6, 'unit': 'TWh', 'panel': 'B'})
    save_figure_data(pd.DataFrame(h2e_rows), output_path, figure_type='multi_panel')
...
def plot_stranded_assets(stranded_df, output_path):
    ...
    save_figure(plt.gcf(), output_path)
    print(f"Saved: {output_path}")
    # CSV export: stranded assets
    sa_rows = []
    for _, row in stranded_df.iterrows():
        sc = row['scenario']
        sa_rows.append({'category': sc, 'subcategory': '1.5C',
                        'value': row['stranded_value_1.5C_usd'] / 1e9, 'unit': 'billion_USD'})
        sa_rows.append({'category': sc, 'subcategory': '2.0C',
                        'value': row['stranded_value_2.0C_usd'] / 1e9, 'unit': 'billion_USD'})
    save_figure_data(pd.DataFrame(sa_rows), output_path, figure_type='grouped_bar')
...
def plot_facility_scatter(df, output_path):
    ...
    save_figure(plt.gcf(), output_path)
    print(f"Saved: {output_path}")
    # CSV export: facility scatter (raw columns rather than generic category/value)
    scenario_data = df[(df['scenario'] == 'shaheen_ncc_h2') & (df['year'] == 2050)]
    scenario_data = scenario_data[scenario_data['mac_usd_per_tco2'] > 0]
    scatter_df = scenario_data[['region', 'mac_usd_per_tco2', 'abatement_tco2']].copy()
    scatter_df['abatement_ktco2'] = scatter_df['abatement_tco2'] / 1e3
    scatter_tidy = scatter_df.rename(columns={'region': 'category', 'mac_usd_per_tco2': 'value'})
    scatter_tidy['unit'] = 'USD_per_tCO2'
    save_figure_data(scatter_tidy[['category', 'value', 'abatement_ktco2', 'unit']], output_path, figure_type='scatter')
...
def main():
    ...
    scenario_summary.to_csv(OUTPUTS_DIR / 'report_scenario_summary.csv', index=False)
    regional_summary.to_csv(OUTPUTS_DIR / 'report_regional_summary.csv', index=False)
    tech_deployment.to_csv(OUTPUTS_DIR / 'report_tech_deployment.csv', index=False)
    print(f"Saved summary tables to {OUTPUTS_DIR}/")
